...
def run_training():
    ...
    # -------------------------------------------------------------------------
    # We use 2048 envs x 32 steps (65,536 transitions per batch) to perfectly saturate
    # the 8.1 TFLOPS capacity of a Kaggle T4 GPU, yielding ~687s per PPO update.
    # This allows ~45 updates within the 8.5-hour safety limit!
    num_envs = 2048
    T_steps = 32
    update_epochs = 4
    num_minibatches = 4
    ...
    # -------------------------------------------------------------------------
    # Kaggle background runs terminate abruptly at 9 hours.
    # We will self-terminate at 8.5 hours (30600s).
    MAX_RUNTIME_SEC = 30600 
    ...
    def train_step(p_params, p_opt_state, c_state, p_rng, update_step):
        ...
        # 1. Rollout
        next_carry_tuple, transitions = generate_trajectory(p_params, (c_states, c_obses, c_rng, r_mean, r_var, r_count), r_mean, r_var)
        next_c_states, next_c_obses, next_c_rng, r_mean, r_var, r_count = next_carry_tuple
        ...
        (next_params, next_opt_state, next_e_rng), epoch_losses = jax.lax.scan(
            epoch_step, 
        ...
        return next_params, next_opt_state, next_c_state, next_e_rng, final_loss, mean_reward, mean_survival
